...
#define DEBUG_LEVEL DEBUG_LEVEL_NONE  /* v2.210: INFO level (DEBUG has compilation errors) */
...
/* v2.244: Removed volatile - use atomic operations for synchronization
 * volatile only prevents compiler optimization, NOT CPU reordering
 * Proper memory ordering requires __atomic_load_n/__atomic_store_n
 */
struct cleanup_queue {
    uint32_t head;  /* Producer writes here (callbacks) - access with atomics */
    uint32_t tail;  /* Consumer writes here (main loop) - access with atomics */
                                                   ...
                                                   uint16_t sport, uint16_t dport)
{
    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (!connection_table[i].active) {
            ...
            __atomic_fetch_add(&connection_count, 1, __ATOMIC_ACQ_REL);  /* v2.244: Atomic increment */
...
static void connection_remove(struct tcp_pcb *pcb)
{
    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (connection_table[i].active && connection_table[i].pcb == pcb) {
            ...
            uint32_t old = __atomic_fetch_sub(&connection_count, 1, __ATOMIC_ACQ_REL);
            if (old == 0) {
 ...
 * Memory ordering:
 *   1. Read tail with ACQUIRE (see consumer's updates)
 *   2. Write slot data (plain stores OK - synchronized by release)
 *   3. Update head with RELEASE (publish to consumer)
 */
static inline void enqueue_cleanup(uint32_t session_id)
{
    ...
    /* Write slot data (plain stores - will be synchronized by release below) */
    ...
    /* RELEASE: Ensure slot writes complete before head update (publish to consumer) */
...
/* v2.244: SPSC Queue Consumer - Process cleanup requests from main loop
 * Memory ordering:
 *   1. Read head with ACQUIRE (see all producer's slot writes)
 *   2. Read slot data (guaranteed fresh due to acquire above)
 *   3. Update tail with RELEASE (allow producer to reuse slot)
 */
static void process_cleanup_queue(void)
{
    uint32_t tail = cleanup_queue.tail;  /* Only consumer writes - plain read OK */
    /* ACQUIRE: Synchronize with producer's head updates - see all slot writes */
    ...
    while (tail != head) {
        ...
        /* Guaranteed to see producer's writes due to ACQUIRE above */
        ...
        if (req->session_id == 0) {
            ...
            tail++;
            /* RELEASE: Publish tail update to producer */
        ...
        if (meta == NULL) {
            ...
            if (meta->metadata_close_pending) {
                ...
                tail++;
                /* RELEASE: Publish tail update to producer (allow slot reuse) */
                __atomic_store_n(&cleanup_queue.tail, tail, __ATOMIC_RELEASE);
        ...
        /* Advance tail - RELEASE to publish update to producer */
        tail++;
        __atomic_store_n(&cleanup_queue.tail, tail, __ATOMIC_RELEASE);
    }
}
...
static void connection_cleanup_atomic(struct connection_metadata *meta)
{
    ...
    /* v2.244: Atomic decrement counters with underflow protection - ONLY place this happens! */
    uint32_t old_count = __atomic_fetch_sub(&connection_count, 1, __ATOMIC_ACQ_REL);
    if (old_count == 0) {
        __atomic_fetch_add(&connection_count, 1, __ATOMIC_ACQ_REL);  /* Restore if underflowed */
        DEBUG_ERROR("%s: connection_count underflow prevented!\n", COMPONENT_NAME);
    ...
    if (old_active == 0) {
        __atomic_fetch_add(&active_connections, 1, __ATOMIC_ACQ_REL);  /* Restore if underflowed */
        DEBUG_ERROR("%s: active_connections underflow prevented!\n", COMPONENT_NAME);
    }
...
static void process_rx_packets(void)
{
     ...
     * v2.244: ATOMIC VERSION - Fix race condition in check-then-set
     *
     ...
     *
     * Previous bug (v2.138-v2.243): Race between check and set
     * - Thread 1: if (in_rx_processing) → reads FALSE
     * - [IRQ fires]
     * - Thread 2: if (in_rx_processing) → reads FALSE
     * - Thread 1: in_rx_processing = TRUE
     * - Thread 2: in_rx_processing = TRUE
     * - BOTH threads enter critical section! ❌
     *
     * Solution (v2.244): Atomic test-and-set eliminates race window
     * - __atomic_test_and_set: atomically reads AND sets in single operation
     * - Returns old value (0=acquired lock, 1=already held)
     * - __ATOMIC_ACQUIRE ensures subsequent reads see consistent memory state
     * - No race window between check and set ✅
     */
    static bool in_rx_processing = false;

    /* Atomic test-and-set: returns TRUE if lock already held */
    if (__atomic_test_and_set(&in_rx_processing, __ATOMIC_ACQUIRE)) {
    ...
    while (loop_count < MAX_PACKETS_PER_CALL) {
        ...
        if (pending_packets == 0) {
            ...
            __atomic_clear(&in_rx_processing, __ATOMIC_RELEASE);  /* v2.244: Atomic clear with release semantics */
            return;
        }
    ...
    /* v2.244: Release reentrancy guard with atomic clear */
    __atomic_clear(&in_rx_processing, __ATOMIC_RELEASE);
}
