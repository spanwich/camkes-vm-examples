...
static void print_frame_metadata(const FrameMetadata *meta) {
    DEBUG_PRINTF("ICS_Inbound: Frame Metadata:\n");
...
static bool validate_message(const ICS_Message *msg) {
    ...
    if (msg->payload_length > MAX_PAYLOAD_SIZE) {
        DEBUG_PRINTF("ICS_Inbound: REJECT - Payload too large (%u > %u)\n",
               msg->payload_length, MAX_PAYLOAD_SIZE);
        return false;
    }

    if (msg->payload_length != meta->payload_length) {
        DEBUG_PRINTF("ICS_Inbound: REJECT - Payload length mismatch (msg=%u, meta=%u)\n",
    ...
    if (msg->payload_length > 0) {
        if (!everparse_validate(msg->payload, msg->payload_length)) {
            DEBUG_PRINTF("ICS_Inbound: REJECT - EverParse validation failed\n");
            return false;
        }
    }
    ...
    DEBUG_PRINTF("ICS_Inbound: ALLOW - Message passed validation\n");
    return true;
}
...
static bool process_message(void) {
    ...
    if (!basic_bounds_check(in_msg, sizeof(Buf))) {
        DEBUG_PRINTF("ICS_Inbound: ERROR - Bounds check failed\n");
    ...
    DEBUG_PRINTF("ICS_Inbound: Forwarded message to internal network\n");
    return true;
}
...
int run(void) {
    DEBUG_PRINTF("ICS_Inbound: Ready to validate external→internal traffic\n");
    DEBUG_PRINTF("ICS_Inbound: Phase 1 - Pass-through with comprehensive logging\n");
