...
# Configuration
ETH0_IP="192.168.95.2"    # Impersonates PLC on internal network
ETH1_IP="192.168.95.1"    # Impersonates router on secure network
...
echo "  Physical NICs:"
echo "    ${ETH0_NAME}: ${ETH0_IP} (impersonates PLC on internal network)"
echo "    ${ETH1_NAME}: ${ETH1_IP} (impersonates router on secure network)"
echo ""
echo "  QEMU Guest IPs:"
echo "    Net0: ${QEMU_NET0_IP} (via tap0 - external/SCADA side)"
echo "    Net1: ${QEMU_NET1_IP} (via tap1 - internal/PLC side)"
echo ""
echo "  Modbus Port: ${MODBUS_PORT}"
echo ""
echo -e "${YELLOW}NOTE: Both NICs use 192.168.95.0/24 but on different vSwitches${NC}"
echo ""
...
echo "  2. Packet arrives at ${ETH0_NAME} (internal network vSwitch)"
...
echo "  6. Exits ${ETH1_NAME} (secure network vSwitch) to real PLC (192.168.95.2)"
echo ""
echo "OUTBOUND (PLC → SCADA):"
echo "  1. Real PLC (192.168.95.2) sends to gateway (${ETH1_IP}:${MODBUS_PORT})"
echo "  2. Packet arrives at ${ETH1_NAME} (secure network vSwitch)"
...
echo "  6. Exits ${ETH0_NAME} (internal network vSwitch) to router → SCADA"
echo ""
