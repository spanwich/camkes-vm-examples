...
static int virtio_net_init(void)
{
    ...
    printf("%s: VirtIO @ SLOT 1 (+0x200): Magic=0x%x, Version=%u, DeviceID=%u\n",
    ...
    printf("%s: ✓ Found VirtIO network device at slot 30\n", COMPONENT_NAME);
...
void post_init(void)
{
    ...
    printf("╚══════════════════════════════════════════════════════════╝\n");
    printf("%s: Ready for bidirectional ICS traffic (OUTBOUND: TCP:7000 → ICS_Outbound)\n", COMPONENT_NAME);
    printf("%s: INBOUND path: ICS_Inbound → TCP (not yet implemented)\n", COMPONENT_NAME);
