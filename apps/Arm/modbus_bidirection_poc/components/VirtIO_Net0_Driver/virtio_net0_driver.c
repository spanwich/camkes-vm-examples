 ...
 *
 * Net0 faces Internal Network (192.168.95.0/24)
 * - Listens on 192.168.95.2:502 (pretends to be PLC)
 * - Forwards validated traffic to real SCADA on 192.168.90.5:502
 *
 * This creates transparent man-in-the-middle for SCADA→PLC traffic
 */
#define OUTBOUND_FORWARD_IP "192.168.90.5"    /* Real SCADA IP */
...
void post_init(void)
{
    ...
    IP4_ADDR(&ipaddr, 192, 168, 95, 2);    /* Static IP: 192.168.95.2 (PLC address) */
    ...
    printf("%s:   IP:      192.168.95.2 (pretends to be PLC)\n", COMPONENT_NAME);
    ...
    printf("(Port 6000 is forwarded to guest port 6000 - External network)\n\n");
