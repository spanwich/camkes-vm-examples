 ...
 */
#define DEBUG_INFINITE_LOOP 0  /* Enable loop iteration debugging */
...
static void process_rx_packets(void)
{
    ...
    /* CRITICAL: Memory barrier to ensure we read fresh vq->used->idx from shared memory */
    __sync_synchronize();
    uint16_t used_idx_snapshot = vq->used->idx;

    if (used_idx_snapshot == last_used_idx) {
        return;
    }

    while (last_used_idx != used_idx_snapshot) {
...
#if DEBUG_INFINITE_LOOP
        /* CRITICAL DEBUG: Detect infinite loop */
        static uint32_t loop_iteration = 0;
        loop_iteration++;
        if (loop_iteration <= 10 || (loop_iteration % 50) == 0) {
            printf("%s: [LOOP #%u] last_used=%u, used_idx_snapshot=%u, used_ring_idx=%u, desc_idx=%u, len=%u\n",
                   COMPONENT_NAME, loop_iteration, last_used_idx, used_idx_snapshot,
                   used_ring_idx, desc_idx, len);
            printf("%s:   vq=%p, vq->used=%p, &vq->used->idx=%p, snapshot=%u\n",
                   COMPONENT_NAME, (void*)vq, (void*)vq->used,
                   (void*)&vq->used->idx, used_idx_snapshot);
        }

        /* Safety: Break infinite loop after 1000 iterations */
        if (loop_iteration > 1000) {
            printf("%s: ⚠️  INFINITE LOOP DETECTED! Breaking after 1000 iterations.\n", COMPONENT_NAME);
            printf("%s:   last_used_idx=%u never caught up to used_idx_snapshot=%u\n",
                   COMPONENT_NAME, last_used_idx, used_idx_snapshot);
            break;
        }
#else
        /* PRODUCTION: Safety break to prevent infinite loop hanging the system */
        static uint32_t safety_counter = 0;
        safety_counter++;
        if (safety_counter > 1000) {
            printf("%s: ⚠️  SAFETY BREAK! Processed 1000+ packets in single batch.\n", COMPONENT_NAME);
            printf("%s:   This may indicate a virtqueue synchronization bug.\n", COMPONENT_NAME);
            printf("%s:   last_used_idx=%u, used_idx_snapshot=%u\n",
                   COMPONENT_NAME, last_used_idx, used_idx_snapshot);
            safety_counter = 0;  /* Reset for next batch */
            break;
        }
#endif
        ...
        /* Log packet arrival with timestamp */
        uint32_t timestamp_ms = sys_now();
        printf("%s: 📥 RX packet #%u [T=%u.%03us], %u bytes (total %u with header)\n",
               COMPONENT_NAME, packets_received, timestamp_ms / 1000, timestamp_ms % 1000,
               packet_len, len);

        /* Display ASCII payload if present (skip Ethernet/IP/TCP headers) */
        if (packet_len > 54) {  /* Ethernet(14) + IP(20) + TCP(20) = 54 bytes minimum */
            /* Estimate payload offset (rough calculation) */
            uint16_t payload_offset = 54;  /* Basic TCP/IP stack overhead */
            uint16_t payload_len = packet_len - payload_offset;

            if (payload_len > 0 && payload_len < 200) {  /* Sanity check */
                printf("  ASCII Payload (%u bytes): \"", payload_len);
                for (uint16_t i = 0; i < payload_len && i < 100; i++) {
                    unsigned char c = packet_data[payload_offset + i];
                    if (c >= 32 && c <= 126) {
                        printf("%c", c);
                    } else if (c == '\n') {
                        printf("\\n");
...
#if DEBUG_INFINITE_LOOP
        /* CRITICAL DEBUG: Verify index increment */
        uint16_t old_idx = last_used_idx;
        last_used_idx++;
        if (loop_iteration <= 10 || (loop_iteration % 50) == 0) {
            printf("%s: [LOOP #%u] Incremented last_used_idx: %u → %u (vq->used->idx=%u)\n",
                   COMPONENT_NAME, loop_iteration, old_idx, last_used_idx, vq->used->idx);
        }
#else
        last_used_idx++;
#endif
...
void post_init(void)
{
    ...
    /* Countdown - use lwIP timeouts for actual 1-second delays */
    for (int i = 60; i > 0; i--) {
        if (i % 10 == 0 || i <= 5) {
            printf("[T=%d] %d seconds remaining...\n", 60-i, i);
        }
        /* Process lwIP timers during wait (gives accurate 1-second delay) */
        uint32_t start_ticks = sys_now();
        while ((sys_now() - start_ticks) < 1000) {  /* 1000ms = 1 second */
            sys_check_timeouts();
            seL4_Yield();  /* Let other threads run */
        }
    }
    ...
    /* IMPORTANT: DO NOT call setup_tcp_echo_server() here!
     * It will be called from run() after lwIP memory allocator is fully ready.
     * Calling it here causes malloc fault at address 0x10 (NULL + offset).
     */
    printf("%s: ✓ Initialization complete (TCP server will be created in run())\n", COMPONENT_NAME);
    printf("%s: Network ready - waiting for lwIP stabilization...\n\n", COMPONENT_NAME);
...
int run(void)
{
    /* Deferred TCP server initialization (after lwIP memory allocator stabilizes) */
    static uint32_t run_count = 0;

    if (!tcp_server_initialized) {
        run_count++;

        /* Wait for 100 iterations (~100ms with seL4_Yield) to let lwIP stabilize */
        if (run_count >= 100) {
            printf("%s: lwIP stabilized, creating TCP echo server...\n", COMPONENT_NAME);
            setup_tcp_echo_server();
            tcp_server_initialized = true;
            printf("%s: ✅ TCP Echo Server ready on 10.0.2.15:1234\n", COMPONENT_NAME);
            printf("%s: Test with: nc localhost 6000\n\n", COMPONENT_NAME);
        }
    }
